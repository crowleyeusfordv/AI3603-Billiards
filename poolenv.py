...
class PoolEnv():
    ...
    def take_shot(self, action: dict):
        ...
        enemy_pocketed = [bid for bid in new_pocketed if
                          bid not in self.player_targets[self.players[self.curr_player]] and bid not in ["cue", "8"]]
        ...
        if "cue" in new_pocketed and "8" in new_pocketed:
            ...
            return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed, 'WHITE_BALL_INTO_POCKET': True,
                    'BLACK_BALL_INTO_POCKET': True, 'FOUL_FIRST_HIT': False, 'NO_POCKET_NO_RAIL': False,
        ...
        if "cue" in new_pocketed:
            ...
            return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed, 'WHITE_BALL_INTO_POCKET': True,
                    'BLACK_BALL_INTO_POCKET': False, 'FOUL_FIRST_HIT': False, 'NO_POCKET_NO_RAIL': False,
        ...
        if "8" in new_pocketed:
            ...
            if len(remaining_own) == 0:
                ...
                self.winner = self.players[1 - self.curr_player]
            self.done = True
            return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed,
                    'WHITE_BALL_INTO_POCKET': False, 'BLACK_BALL_INTO_POCKET': True, 'FOUL_FIRST_HIT': False,
                    'NO_POCKET_NO_RAIL': False, 'BALLS': copy.deepcopy(self.balls)}
        ...
        if first_contact_ball_id is None:
            ...
            return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed,
                    'WHITE_BALL_INTO_POCKET': False, 'BLACK_BALL_INTO_POCKET': False, 'FOUL_FIRST_HIT': False,
                    'NO_POCKET_NO_RAIL': False, 'NO_HIT': True, 'BALLS': balls_before_shot}
        if first_contact_ball_id is not None:
            opponent_plus_eight = [bid for bid in self.balls.keys() if
                                   bid not in self.player_targets[player] and bid not in ['cue']]
            ...
            if (len(remaining_own_before) > 0 and first_contact_ball_id in opponent_plus_eight) or \
                ...
                if self.hit_count >= self.MAX_HIT_COUNT:
                    ...
                    self.done = True
                    a_left = len(
                        [bid for bid in self.player_targets["A"] if bid != '8' and self.balls[bid].state.s != 4])
                    b_left = len(
                        [bid for bid in self.player_targets["B"] if bid != '8' and self.balls[bid].state.s != 4])
                    if a_left < b_left:
                        self.winner = "A"
                    elif b_left < a_left:
                        self.winner = "B"
                    ...
                    print(f"📊 最大击球数详情：A剩余 {a_left}，B剩余 {b_left}，胜者：{self.winner}")
                return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed,
                        'WHITE_BALL_INTO_POCKET': False, 'BLACK_BALL_INTO_POCKET': False, 'FOUL_FIRST_HIT': True,
                        'NO_POCKET_NO_RAIL': False, 'BALLS': copy.deepcopy(self.balls)}
        ...
        if len(new_pocketed) == 0:
            if (not cue_hit_cushion) and (not target_hit_cushion):
                ...
                return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed,
                        'WHITE_BALL_INTO_POCKET': False, 'BLACK_BALL_INTO_POCKET': False, 'FOUL_FIRST_HIT': False,
                        'NO_POCKET_NO_RAIL': True, 'BALLS': balls_before_shot}
            else:
                # 无进球但有球碰库，仅交换球权
        ...
        if self.hit_count >= self.MAX_HIT_COUNT:
            ...
            print(f"📊 最大击球数详情：A剩余 {a_left}，B剩余 {b_left}，胜者：{self.winner}")
            return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed,
                    'WHITE_BALL_INTO_POCKET': False, 'BLACK_BALL_INTO_POCKET': False,
                    'BALLS': copy.deepcopy(self.balls)}

        # return 一些这一杆的结果信息
        return {'ME_INTO_POCKET': own_pocketed, 'ENEMY_INTO_POCKET': enemy_pocketed, 'WHITE_BALL_INTO_POCKET': False,
                'BLACK_BALL_INTO_POCKET': False, 'FOUL_FIRST_HIT': False, 'NO_POCKET_NO_RAIL': False,
                'BALLS': copy.deepcopy(self.balls)}
